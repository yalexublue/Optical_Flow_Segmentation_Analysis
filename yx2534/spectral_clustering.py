...
#test data, 2 clusters is initiated here for two circles of dots.
X, clusters = make_circles(n_samples=1000, noise=.05, factor=.5, random_state=0)
J = np.zeros((len(X), 3), float)
W = pairwise_distances(X, metric="euclidean")


cluster = spectral_clustering(df=W, n_neighbors=8, n_clusters=2)
print('affinity matrix:')
print(W)
print (X)
count = 0
for i in range (0, len(X)):
    J[i] = np.append(X[i], [cluster[count]])
    count += 1
print(J)
...
fig, ax = plt.subplots()
sns.scatterplot(x=J[:, 1], y=J[:, 0], data=J, hue=J[:, 2], ax=ax)
